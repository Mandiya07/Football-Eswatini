...
const CupsPage: React.FC = () => {
  ...
  const handleBack = () => {
      if (currentView === 'bracket') {
          ...
          const isSlotMember = currentCup?.hubSlot && currentCup.hubSlot !== 'trade-fair';
          if (name.includes('ingwenyama') || isSlotMember) setCurrentView('ingwenyama-hub');
  ...
  const renderHub = () => {
      const tradeFairCup = cups.find(c => c.hubSlot === 'trade-fair');
      ...
      return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto animate-fade-in">
              ...
              <Card 
                className={`group cursor-pointer hover:shadow-2xl transition-all duration-300 border-t-4 border-blue-600 bg-gradient-to-br from-white to-blue-50 ${!tradeFairCup ? 'opacity-50 grayscale' : ''}`}
                onClick={() => tradeFairCup && handleCupSelect(tradeFairCup.id)}
              >
                  <CardContent className="p-8 flex flex-col items-center text-center h-full">
                      ...
                      <p className="text-gray-600 mb-6 flex-grow">{tradeFairCup ? 'The traditional season-opening tournament.' : 'Awaiting tournament setup...'}</p>
                      <span className="text-blue-700 font-bold flex items-center gap-2">
                          {tradeFairCup ? 'View Bracket' : 'Coming Soon'} <ArrowRightIcon className="w-5 h-5 group-hover:translate-x-1 transition-transform"/>
                      </span>
                  </CardContent>
              </Card>
          </div>
  ...
  const renderIngwenyamaHub = () => {
      const getSlotCupId = (slot: string) => cups.find(c => c.hubSlot === slot)?.id;
      
      const regions = [
          { slot: 'hhohho', label: 'Hhohho Region', color: 'text-blue-600', bg: 'bg-blue-50' },
          { slot: 'lubombo', label: 'Lubombo Region', color: 'text-green-600', bg: 'bg-green-50' },
          { slot: 'shiselweni', label: 'Shiselweni Region', color: 'text-orange-600', bg: 'bg-orange-50' },
          { slot: 'manzini', label: 'Manzini Region', color: 'text-red-600', bg: 'bg-red-50' },
      ];

      const nationalCupId = getSlotCupId('national');
      ...
      return (
          <div className="animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                  <Card 
                    className={`sm:col-span-2 lg:col-span-3 cursor-pointer hover:shadow-xl transition-shadow bg-gray-900 text-white border-b-4 border-yellow-500 ${!nationalCupId ? 'opacity-60 grayscale' : ''}`}
                    onClick={() => nationalCupId && handleCupSelect(nationalCupId)}
                  >
                      <CardContent className="p-6 flex items-center justify-between">
                          <div>
                              <h3 className="text-2xl font-bold font-display text-yellow-400">National Finals (Main Draw)</h3>
                              <p className="text-gray-300 text-sm mt-1">{nationalCupId ? 'The Grand Finale featuring elite qualified clubs.' : 'Awaiting official national draw...'}</p>
                          </div>
                          <ArrowRightIcon className="w-8 h-8 text-white" />
                      </CardContent>
                  </Card>
                  {regions.map(region => {
                      const cid = getSlotCupId(region.slot);
                      return (
                        <Card 
                            key={region.slot} 
                            className={`cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1 ${!cid ? 'opacity-40 grayscale pointer-events-none' : ''}`} 
                            onClick={() => cid && handleCupSelect(cid)}
                        >
                            <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">{cid ? 'View Live Qualifiers' : 'Not yet active'}</p>
                            </CardContent>
                        </Card>
                      );
                  })}
