...
const StandingsTable: React.FC<StandingsTableProps> = ({ standings }) => {
    ...
    const validStandings = (standings || []).filter((t): t is Team => !!(t && t.stats && typeof t.stats.form === 'string'));

    if (validStandings.length === 0) {
        return <p className="text-sm text-gray-500 italic">No group standings available at the moment.</p>;
    }
