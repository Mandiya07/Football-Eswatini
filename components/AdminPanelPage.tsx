...
const EcosystemAgents = lazy(() => import('./admin/EcosystemAgents'));
const SalesCRM = lazy(() => import('./admin/SalesCRM'));

type AdminTab = 'approvals' | 'users' | 'news' | 'shop' | 'scouting' | 'directory' | 'videos' | 'ads' | 'create' | 'merge' | 'standings' | 'tournament' | 'categories' | 'reset' | 'teams' | 'live' | 'matches' | 'seed' | 'youth' | 'features' | 'referees' | 'social' | 'crests' | 'community' | 'contracts' | 'international' | 'maintenance' | 'inquiries' | 'insights' | 'agents' | 'crm' | 'season' | 'podcasts';
...
const AdminPanelPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<AdminTab>(isRefereeAdmin ? 'referees' : 'approvals');
  ...
  if (!isLoggedIn || (!isSuperAdmin && !isRefereeAdmin && !isLeagueAdmin)) {
    return (
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'season': return <SeasonManagement />;
      ...
      case 'agents': return <EcosystemAgents />;
                ...
                <aside className="w-full md:w-64 flex-shrink-0">
                    <div className="space-y-2 bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                        {isRefereeAdmin ? (
                            ...
                            <>
                                <h4 className="font-bold text-xs uppercase text-gray-400 px-4 pt-2">League Hub</h4>
                                <TabButton tabName="matches" label="Manage Matches" Icon={CalendarIcon} />
                                <TabButton tabName="season" label="Season Management" Icon={CalendarIcon} />
                            ...
                            <>
                                ...
                                <Link to="/admin/efa-dashboard" className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-blue-700 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                                    <ShieldIcon className="w-5 h-5 text-blue-600" />
                                    <span>EFA Strategic Dashboard</span>
                                </Link>
                                ...
                                <TabButton tabName="agents" label="AI Ecosystem Agents" Icon={Cpu as any} className="bg-purple-50 text-purple-700 hover:bg-purple-100" />
                                <TabButton tabName="podcasts" label="AI Podcasts" Icon={MicIcon} className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100" />
                                ...
                                <h4 className="font-bold text-xs uppercase text-gray-400 px-4 pt-4">Sales & Subscriptions</h4>
                                <TabButton tabName="crm" label="Sales CRM Portal" Icon={BriefcaseIcon} className="bg-emerald-50 text-emerald-800 hover:bg-emerald-100" />
